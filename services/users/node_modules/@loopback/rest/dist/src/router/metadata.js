...
exports.api = api;
function getEndpoints(target) {
    let endpoints = context_1.Reflector.getOwnMetadata(ENDPOINTS_KEY, target);
    if (!endpoints) {
        // Clone the endpoints so that subclasses won't mutate the metadata
        // in the base class
        const baseEndpoints = context_1.Reflector.getMetadata(ENDPOINTS_KEY, target);
...
function getControllerSpec(constructor) {
    let spec = context_1.Reflector.getOwnMetadata(API_SPEC_KEY, constructor);
    if (!spec) {
